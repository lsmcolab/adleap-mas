...
class POMCP(object):
    ...
    def __init__(self,max_depth,max_it,kwargs):
        ...
        # POMCP enhancements
        ###
        # Black-box simulation (silver2010pomcp)
        black_box_simulation = kwargs.get('black_box_simulation')
        if black_box_simulation is not None:
            self.black_box_simulation = black_box_simulation
            if self.black_box_simulation:
                k = kwargs.get('k')
                self.k = k if k is not None else 100
        else: #default
            self.black_box_simulation =  True
            k = kwargs.get('k')
            self.k = k if k is not None else 100
        ...
        if particle_revigoration is not None:
            self.particle_revigoration = particle_revigoration
        else: #default
            self.particle_revigoration = True
    ...
    def rollout(self,node):
        ...
        tmp_state = node.state.copy()
        action = self.rollout_policy(tmp_state)
        ...
        # 4. Rolling out
        return reward +\
        self.discount_factor*self.rollout(observation_node)
    ...
    def black_box_update(self,env,agent,root):
        ...
        elif len(root.particle_filter) > 1:
            while(len(root.particle_filter) < self.k):
                sampled_env = random.sample(root.particle_filter,1)[0]
                root.particle_filter.append(sampled_env)
    ...
    def planning(self, state, agent):
        ...
        if self.black_box_simulation:
            self.black_box_update(state,agent,root_node)
...
def pomcp_planning(env, agent, max_depth=20, max_it=100, **kwargs):    
    ...
    if env.episode == 1:
        print('BB:',pomcp.black_box_simulation,' - PR:',pomcp.particle_revigoration)
    next_action, search_tree = pomcp.planning(copy_env,agent)
